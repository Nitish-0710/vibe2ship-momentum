...
/**
 * Analytics Service.
 * Computes structured productivity analytics for the user from Firestore collections.
 *
 * References: TDD §31, AISPEC §31 "Learning Strategy", PRD Phase 4
 */
...
async function getUserAnalytics(userId) {
  ...
  try {
    // 1. Fetch Tasks
    const taskSnap = await db.collection('tasks').where('userId', '==', userId).get()
    const tasks = taskSnap.docs.map((doc) => doc.data())

    // 2. Fetch Insights
    const insightSnap = await db.collection('insights').doc(userId).get()
    const insightDoc = insightSnap.exists ? insightSnap.data() : null
    ...
    const reflectionSnap = await db
      ...
      .limit(7)
      .get()
    const reflections = reflectionSnap.docs.map((doc) => doc.data())
